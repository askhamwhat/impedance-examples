%Constant-Multiplied Impedance Data Inverse Test
%
% purpose: check that the utilities work for basic impedance problems
%   this test first takes the impedance data and multiplies it by 
%   a random constant at each frequency. this tests the constphasefac
%   utilities
% 
...
u_meas1 = cell(length(u_meas),1);
facs = zeros(length(u_meas),1);
for i = 1:length(u_meas)
    fac = randn()+1i*randn();
    u_meas1{i} = u_meas{i};
    u_meas1{i}.uscat_tgt = u_meas1{i}.uscat_tgt*fac;
    facs(i) = fac;
end
...
[inv_data_all,src_info_out] = rla.rla_inverse_solver(u_meas1,bc,...
                          optim_opts,opts,src_init,lam);

fnameout = [fnamebase, '_timesconst_', bc.invtype, '_invsol', char(datetime), '.mat'];                      
save(fnameout,'inv_data_all','fname','-v7.3','bc','optim_opts','opts','u_meas1','facs');
